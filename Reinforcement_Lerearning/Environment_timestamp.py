...
class TurningUpControlEnv(gym.Env):
    ...
    """
    def __init__(self, state_queue, reward_queue, actions, environment_params, time_logger):
    ...
    def reset(self):
        ...
        self.current_state = np.zeros((250, 3), dtype=np.float32)
    ...
    def step(self, action):
        # アクションに応じてG-codeを送信
        gcode = str(self.actions[action])
        self.send_gcode_threaded(gcode)
    ...
    def transform_distance_to_reward(self, distance):
        mean_first_5 = np.mean(distance[:5])
        mean_last_5  = np.mean(distance[-5:])
        diff = mean_last_5 - mean_first_5

        reward = 1.0 / (diff + 1e-3)
        return reward
    ...
    def step_episode_equalstep(self, action):
        self.time_logger.info("------------------Env_process_init----------------\n")
        # print("action : ", action)
        ...
        print("\n--- Queue Timestamp Information ---")
        # print(states)
        states = self.display_queue_time_range(states, "State Queue")
        rewards = self.display_queue_time_range(rewards, "Reward Queue")
        ...
        self.time_logger.info("------------------Env_process_end----------------\n")
    ...
    def init_procedure(self):
        ...
        init_gcodes = [
            "G28 X", "G28 Z", "G91",
            "G1 Z8 F9000", "G1 X20 F9000", "G1 Z4 F9000"
        ]
    ...
    def display_queue_time_range(self, mp_queue, queue_name):
        """
        キュー内の全アイテムのタイムスタンプの最小値・最大値を表示し、
        タイムスタンプ部分を除いた np.array のみのリストを返す関数。

        Args:
            mp_queue (multiprocessing.Queue): タイムスタンプ付きのデータが入っているキュー
            queue_name (str): キュー名（表示用）

        Returns:
            list: キュー内の各アイテムからタイムスタンプを除いた np.array のみのリスト
        """
        import queue  # キューのEmpty例外用

        temp_items = mp_queue
        sensor_data_list = []
        # キューからすべてのアイテムを一時的に取得

        if temp_items:
            # 各アイテムのタイムスタンプを抽出し、最小・最大を表示
            timestamps = [item[0] for item in temp_items]
            ...
            print(f"[{queue_name}] Data acquired from {start_str} to {end_str}")
            # タイムスタンプを除いたセンサデータのみのリストを作成
            sensor_data_list = [item[1] for item in temp_items]
        else:
            print(f"[{queue_name}] is empty.")
