...
def sensor_process(state_queue, reward_queue):
    ...
    def transform_distance_to_reward(distance):
    # 逆変換: 小さい距離値に対して高い報酬を与える
        return 1.0 / (distance + 1e-3)  # 小さいオフセットを加えて無限大を防止
        return distance
    ...
    try:
        ...
        while True:
            ...
            if touch_data:
                ...
                # logging.debug(f"state_queueに状態を追加: {state}")
                ...
                if touch_counter >= 50:
                    ...
                    # print(distance)
                    if distance is not None:
                        # reward = transform_distance_to_reward(distance)
                        reward_queue.put(distance)
                        # logging.debug(f"reward_queueに報酬を追加: {distance}")
                    else:
                        # 測距センサデータが取得できない場合のデフォルト報酬
                        reward = -0.5
                        reward_queue.put(reward)
                        logging.debug(f"distanceデータ取得失敗のためデフォルト報酬を追加: {reward}")

                    touch_counter = 0  # カウンターをリセット

            time.sleep(0.00125)  # スリープ間隔を1.25msに設定（約800Hz）
