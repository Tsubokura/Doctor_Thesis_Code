...
class Logger():
    ...
    def __init__(self, log_dir="", dir_name="", project_name="pytorch_project"):
        ...
        # wandb.init(project=project_name, dir=self.log_dir)
    ...
    #     # WandBへのメトリクスの記録（ステップ番号を指定しない）
    #     wandb.log({name: value})

    def plot_histogram(self, elapsed_times, name):
        """
        指定されたelapsed_timesリストのヒストグラムをプロットし、wandbにアップロードする。

        Args:
            elapsed_times (list): 時間計測データのリスト。
            name (str): ヒストグラムのタイトルに使用する名前。
        """
        plt.figure(figsize=(8, 6))
        plt.hist(elapsed_times, bins=50, color='skyblue', edgecolor='black', alpha=0.7)
        ...
        # ヒストグラムをwandbにアップロード
        wandb.log({f"{name} Histogram": wandb.Image(plt)})
        
        # プロットを閉じてメモリを解放
        plt.close()

    def summarize_times(self, elapsed_times, name, bin_size=0.1, log_scale=True):
        """
        処理時間のリストをカスタムビンに分け、各ビンのカウントを計算します。
        計算結果をwandbに保存します。

        Args:
            time_list (list of float): 処理時間（秒）のリスト。
            distribution_name (str): 分布の名前。
            project_name (str, optional): wandbのプロジェクト名。デフォルトは 'processing_time_stats'。

        Returns:
            dict: ビンの範囲と対応するカウントの辞書。
        """

        # 処理時間が0より大きいことを確認
        time_array = np.array(elapsed_times)
        if np.any(time_array <= 0):
            raise ValueError("処理時間はすべて正の値である必要があります。")

        bin_edges = [0.001, 0.01, 0.1, 1.0]  # 0.001〜0.01, 0.01〜0.1, 0.1〜1.0秒

        # ヒストグラムを計算
        counts, _ = np.histogram(time_array, bins=bin_edges)

        # ビンの範囲を表す文字列を作成
        bin_labels = [
            f"{bin_edges[i]:.3f}-{bin_edges[i+1]:.3f}" for i in range(len(bin_edges)-1)
        ]

        # 結果を辞書にまとめる
        distribution = {bin_labels[i]: int(counts[i]) for i in range(len(counts))}

        # テーブルを作成
        table = wandb.Table(columns=["Bin Range (s)", "Count"])
        for bin_range, count in distribution.items():
            table.add_data(bin_range, count)

        # テーブルをwandbにログ
        wandb.log({f"{name} Distribution": table})
